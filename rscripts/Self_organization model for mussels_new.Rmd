---
title: "Self-organization model for mussel"
author: "Chen Yuheng"
date: "2024-10-17"
...
```{r}
# Spatially explicit model for regular pattern formation in mussel beds
...
f       =  100        # m3/m3/h  Change rate with upper layer  (Guestimated)
...
# --- An "Open window" function that works both on Mac and on Windows ----------
```{r}
...
TimeRec = MusselRec = d_M_Rec = vector(length=NoFrames) # Mean mussel biomass
```
...
# Initial values for the algae and the mussels
A[,]=0.5 
...
Time =  0          # Begin time 
ii   =  1e6        # Setting the plot counter to max, so that drawing starts immediately
jj = 0
```

# --- Setting up the figure (For mac) ---------------------------------------------------
```{r}
## Open a graphics window (Darwin stands for a Mac computer)
OpenWindow(width=WinWidth, height=WinHeight) # title = 'Self-organization in mussel beds'
...
Plot the relationship between biomass and time
```{r}
plot(d_M_Rec, MusselRec, ylim=c(0,500))

TimeRec <- as.data.frame(TimeRec)
MusselRec <- as.data.frame(MusselRec)
relationship.biomass <-data.frame(TimeRec, MusselRec)
p <- ggplot2::ggplot(data = relationship.biomass, aes(TimeRec, MusselRec)) +
  geom_point() +
  # geom_smooth() +
  theme_bw() +
  xlab('Time') + ylab('Mueesls biomass')
p
```