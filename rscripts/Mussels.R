...
d_dx = function (w) { # fluxes in y-direction, backwards difference scheme
  # Flux = (middle cell - left side cell) / cell size in y dimension
  fx = (w[c(1:m),1:m] - w[c(m,1:(m-1)),1:m]) / dx
  return(fx)
}
...
d2_dxy2 = function (w) { # Diffusion terms in x and y dimensions
  ...
  fxy = (w[c(2:m,1),1:m] + w[c(m,1:m-1),1:m] - 2*w[1:m,1:m])/dx/dx +
  
  #        Above + Below - 2*middle cells        
        (w[1:m,c(2:m,1)] + w[1:m,c(m,1:m-1)] - 2*w[1:m,1:m])/dy/dy;
